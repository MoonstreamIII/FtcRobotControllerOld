...
@Autonomous(name="NONAME Auto Test",group="Linear")
//@Disabled
public class NONAME_Auto_CustomTest extends LinearOpMode {
   ...
   private DcMotor spinner=null; //Carousel spinner motor
    ...
    private String[] commands = CustomAuto.testAuto.commands;
    private double[] values = CustomAuto.testAuto.values;
    private double[] times = CustomAuto.testAuto.times;
    private int[] encodes = CustomAuto.testAuto.encodes;
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            if (curEncode>0) {
                if (curEncode>ldrive.getCurrentPosition()) {
                    instNum++;
                    ldrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                    ldrive.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
                    instTimer.reset();
                ...
                if (curTime > instTimer.milliseconds()) {
                    instNum++;
                    ldrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                    ldrive.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
                    instTimer.reset();
            ...
            dashboardTelemetry.addData("curInst", curInst);
            dashboardTelemetry.addData("instNum", instNum);
            dashboardTelemetry.addData("curTime", curTime);
            dashboardTelemetry.addData("curEncode",curEncode);
            dashboardTelemetry.addData("motorPos",ldrive.getCurrentPosition());
            dashboardTelemetry.addData("time", instTimer.milliseconds());
